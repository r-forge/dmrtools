...
#Modified by zekai otles <otles@fstrf-wi.org>
#I have picked up this code from somewhere where I don't recall but I have modified to use
#my purpose
dmRlegend <-function(x, y = NULL, legend, fill=NULL, col = "black", lty, lwd, pch,
...
## the 2nd arg may really be `legend'
    ...
    #zekai Otles modified to have a bigger box size to get shading right
     if(mfill){
    	fsScale<-dmRlegScales(par)
    xbox<-xbox*(1+fsScale$legendBoxScale)
    ybox<-ybox*(1+fsScale$legendBoxScale)
      } else{
        xbox<-0
        ybox<-0
      }
